...
The local operating-system user, Host Agent, and processes with job-directory write access are trusted. Before review, v1 builds and binds the trusted candidate snapshot. After review, validate the bound source, glossary, trusted candidate snapshot and exported review directly; do not reconstruct upstream analysis, translation exchanges or preview layout. The workflow detects accidental corruption, missing or stale artifacts, cross-job exchange, and uncoordinated changes using SHA-256, strict schemas, exact job binding, stable input snapshots, path/reparse checks, and before/after digest checks. It does not defend against the same user deliberately or coordinately rewriting all job files, and uses no HMAC, external secret, credential store, or external trust record.
